...
public class Out {
    ...
    public static void main(String[] args) {
        String dbName = "visitor_v4_2";
        String outFile = "/Users/lvxuhong/Desktop/小微.xlsx";
        String author = "陈方翔";
        DruidPlugin dp = new DruidPlugin("jdbc:mysql://20.21.1.102/information_schema?serverTimezone=Hongkong", "root", "zjrc2019");
        ActiveRecordPlugin arp = new ActiveRecordPlugin(dp);
        dp.start();
        arp.start();
        List<Record> records = find("select TABLE_NAME,TABLE_COMMENT from TABLES where  TABLE_TYPE = 'BASE TABLE' AND TABLE_SCHEMA = '" + dbName + "'");
        ...
        for (int i = 0; i < records.size(); i++) {
            ...
            List<Record> columns = find("select COLUMN_NAME,COLUMN_COMMENT,DATA_TYPE,IS_NULLABLE,CHARACTER_MAXIMUM_LENGTH from COLUMNS where TABLE_NAME = '" + tableName + "' and TABLE_SCHEMA = '" + dbName + "' order by ORDINAL_POSITION");
            writer.setSheet(i + 1);
